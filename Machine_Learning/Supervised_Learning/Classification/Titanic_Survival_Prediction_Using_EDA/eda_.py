...
def save_plot_if_requested(filename):
    response = input("Press 's' then Enter to save this plot screenshot, or just Enter to continue: ").strip().lower()
    if response == 's':
...
save_plot_if_requested('missing_values_before_cleanup.png')
plt.show()
...
ms.bar(df,figsize = (10,5),color="tomato")
plt.title("Bar plot showing missing data values", size = 15,c="r")
save_plot_if_requested('missing_values_after_cleanup.png')
plt.show()
...
save_plot_if_requested('survival_countplot.png')
plt.show()
...
save_plot_if_requested('sex_distribution.png')
plt.show()

sns.catplot(x="Sex",hue="Survived", kind="count",data=df,height=3,)
save_plot_if_requested('sex_survived_count.png')
plt.show()



sns.countplot(x="Pclass", hue="Survived", data=df, palette="Reds",)
save_plot_if_requested('pclass_survival_count.png')
plt.show()
